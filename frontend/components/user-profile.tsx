...
export function UserProfile() {
    ...
    const [iziToast, setIziToast] = useState<any>(null);

    // Dynamically import iziToast only on client side
    useEffect(() => {
        const loadIziToast = async () => {
            const iziToastModule = await import('izitoast');
            await import('izitoast/dist/css/iziToast.min.css');
            setIziToast(iziToastModule.default);
        };
    ...
    const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file || !userData || !iziToast) return;
    ...
    useEffect(() => {
        // We need to make sure we're on the client side before accessing localStorage
